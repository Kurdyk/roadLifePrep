...
class Intervention(db.Model):
	...
	road_id = db.Column(db.String(36), nullable=False)
	road_url = db.Column(db.String(100), nullable=False)
	...
	def serialize(self):
		return {
			"id": self.id,
			"roadId": self.road_id,
			"roadUrl": self.road_url,
			"description": self.description,
			"refusalDescription": self.refusal_description,
			"askDate": self.date_ask,
			"refusalDate": self.date_refusal,
			"acceptanceDate": self.date_validation,
			"state": self.state,
			"realisationDate": self.date_solved,
			"wearGain": self.gain,
