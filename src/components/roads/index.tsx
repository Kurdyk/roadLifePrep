...
const RoadComponent: React.FC = () => {
    ...
    const {columns, gridDisplayableRoads, visualRoads, visualSensors, isLoading} = useData();

    return (
        <Box id="SwitchableRoadDisplay">
            <SwitchableComponent
                components={[
                    {element:<SearchableDataGridComponent rows={gridDisplayableRoads} columns={columns} id={"RoadGrid"}
                     loading={isLoading}/>, label:"Liste"},
